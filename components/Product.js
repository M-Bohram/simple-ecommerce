...
export default function Product({ name, image, category, price, availableQuantity }) {
    ...
    return (
        <div className={styles['product-container']}>
            ...
            <div className={styles["product-info"]}>
                <p className={styles["product-quantity"]}>Available Qty: <span>{availableQuantity}</span></p>
                <h3 className={styles["product-price"]}>{convertedPrice}</h3>

            </div>
            <div className={styles["add-cart-button"]} onClick={() => dispatch(addToCart({ name, image, category, price, availableQuantity }))}>
